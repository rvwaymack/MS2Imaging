...
if isfield(FrameInfo,'FileMode')
    ...
    elseif strcmp(FrameInfo(end).FileMode,'LSM')||strcmp(FrameInfo(end).FileMode,'LSMExport')||...
            strcmp(FrameInfo(end).FileMode,'LIFExport')||strcmp(FrameInfo(end).FileMode,'LAT')||strcmp(FrameInfo(end).FileMode,'DSPIN')
        for j=1:length(FrameInfo)
        ...
        if (Particles{ChN}(i).Approved==1)
            ...
            if AnalyzeThisParticle
                ...
                if ~SkipTraces
                    ...
                    %For Nikon spinning disk data plot Top Left plot on two
                    %y axes because the offset is very high. CS20170915    
                    if strcmp(FrameInfo(1).FileMode,'DSPIN')  
                        x = ElapsedTime(CompiledParticles{ChN}(k).Frame);
                        y1 = CompiledParticles{ChN}(k).Fluo;
                        errorY1 = ones(size(CompiledParticles{ChN}(k).Fluo))*CompiledParticles{ChN}(k).FluoError;
                        y2 = CompiledParticles{ChN}(k).Off;  %CS removed *IntArea
                        
                        yyaxis left
                        errorbar(x,y1,errorY1,'.-');
                        ylim([0 max([CompiledParticles{ChN}.Fluo])])
                        ylabel('Fluorescence (au)')
                        hold on                   
                        
                        yyaxis right
                        plot(x,y2,'.-');
                        ylim([max([CompiledParticles{ChN}.Off])-15, max([CompiledParticles{ChN}.Off])])   %CS removed *IntArea
                        ylabel('Offset Fluorescence (au)')
                        xlabel('Time (min)')
                    ...
                    elseif strcmp(FrameInfo(1).FileMode,'LSM')||strcmp(FrameInfo(1).FileMode,'LIFExport')||strcmp(FrameInfo(1).FileMode,'LAT')||strcmp(FrameInfo(1).FileMode,'TIF')
                        ...
                        if ~isempty(x)
                            ...
                            if NChannels==1
                                Image=imread([PreProcPath,filesep,FilePrefix(1:end-1),filesep,...
                                    FilePrefix,iIndex(CompiledParticles{ChN}(k).Frame(j),NDigits),'_z',iIndex(zTrace,2),'_ch01','.tif']);   %CS20171204
                            else
                                Image=imread([PreProcPath,filesep,FilePrefix(1:end-1),filesep,...
