...
elseif strcmp(FileMode,'DSPIN')||strcmp(FileMode,'DND2')
    ...
    backgd = 200;       %Camera offset on the Nikon spinning disk is 200. This must be subtracted from all images and the FF before doing FF correction. 
    ...
    if exist('DSPINFF')
        ...
        [~,ChannelToUse]=max(MaxValue);
        imwrite((DSPINFF{1}{ChannelToUse,1}-backgd),[OutputFolder,filesep,Prefix,'_FF.tif']);  %Must subtract the camera offset (background=200)
    end
        ...
        for iTP = 1:NTimePoints(iSeries)             %for each timepoint in this series
            ...
            if strcmp(FileMode,'DSPIN')  % .nd files list images as tp1: Ch1 z1:21, Ch2 z1:21 before moving to tp2
                ...
                for ii = firstImage:lastImage
                    ...
                    imwrite((DSPINimages{iSeries}{ii}-backgd),[OutputFolder,filesep,NewName]);
                    n = n+1;
                end
